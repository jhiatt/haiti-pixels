...
```{r model-evaluation-pr, results="hide"}
# adding calculated parameters
...
I noticed that Holdout 2 and Holdout 3 have the same number of observations.  Looking closer, I noticed that they both reference region 67 blue tarps, which makes me thing this is duplicate data in a different format.  To be sure, I will compare the B1, B2, and B3 values.
...
Rounding down for the average values, the Blue Tarp gave a mean and a median that were considerably more blue than the Other group of values.  Both the mean and median for the Blue Tarps gave a bluish grey, while the, mean for Other was a dark gold and the median was a dark brown.  This lines up with my expectations so I am confident in assuming that B1, B2, and B2 are equivalent to Red, Green, Blue.  I will go ahead and update the data frame, and I will convert Class.cl to a factor.
...
generate_predictions <- function(data) {
  ...
  # svg linear
  svg.linear.predictions <- svm.linear.fit.final %>% predict(data, probability=TRUE)
  
  #svg radial
  svg.radial.predictions <- svm.radial.fit.final %>% predict(data, probability=TRUE)
  ...
  return(data_frame(actual=data$Class.cl,
                    ...
                    svg_linear_prediction=1-attr(svg.linear.predictions,"probabilities")[,2],
                    svg_radial_prediction=1-attr(svg.radial.predictions,"probabilities")[,2]))
}
```

```{r cache=TRUE}
d <- data.holdout.1 %>% mutate(Red = B1, Green = B2, Blue = B3)
d$Class.cl <- d$Class.cl %>% as_factor()
data.holdout.results <- d %>% generate_predictions()
...
```{r holdout-evaluation, results="hide"}
...
holdout_results
holdout_results <- holdout_results %>% mutate(Tuning = "", AUROC = 0) %>% column_to_rownames(var = "Model")
...
holdout_results <- holdout_results %>% dplyr::select(Tuning, AUROC, Threshold, Accuracy, TPR, FPR, Precision)
holdout_results
```

```{r holdout-evaluation-pr, results="hide"}
# adding calculated parameters
pr.df1 <- evaluate_model("Logisitic",logistic.thresh.pr$.threshold,logistic.pred$prediction,logistic.pred$actual)
pr.df1.hp <- evaluate_model("Logisitic - Hand Picked",logistic.thresh.pr.hp$.threshold,logistic.pred$prediction,logistic.pred$actual)
pr.df2 <- evaluate_model("LDA",lda.thresh.pr$.threshold,lda.pred$prediction,lda.pred$actual)
...
holdout_results_pr <- rbind(pr.df1, pr.df1.hp, pr.df2, pr.df3, pr.df3.hp, pr.df4, pr.df5, pr.df6, pr.df7, pr.df8)
holdout_results_pr
holdout_results_pr <- holdout_results_pr %>% mutate(Tuning = "", AUROC = 0) %>% column_to_rownames(var = "Model")
...
holdout_results_pr <- holdout_results_pr %>% dplyr::select(Tuning, AUROC, Threshold, Accuracy, TPR, FPR, Precision)
holdout_results_pr
```
