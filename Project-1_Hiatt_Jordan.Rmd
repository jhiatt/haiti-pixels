...
cv_results <- rbind(df1, df2, df3, df4, df5, df6, df7, df8)
cv_results
cv_results <- cv_results %>% mutate(Tuning = "", AUROC = 0) %>% column_to_rownames(var = "Model")

model_types <- list("logistic","lda","qda","knn","elnet","rf","svm.linear","svm.radial")
...
# update thresholds for custom picked thresholds
# copy for PR thresholds
# Ensure all thresholds are fixed.
# add tuning for rf and svm
# check table results

# get actual and prediction values for holdout data
# copy and paste these with that data
# remove output from this table
```
```{r, echo=FALSE, results="asis"}
kable(cv_results)
```
